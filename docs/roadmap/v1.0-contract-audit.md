...
`ROADMAP.md` must reflect the current released `0.38.x` line. Choice union mode, JSON array streaming, conformance gate wiring, baseline WSDL coverage, wildcard attribute bags, and the dependency refresh must be treated as shipped work, while final release-candidate gates remain the active 1.0 work.
